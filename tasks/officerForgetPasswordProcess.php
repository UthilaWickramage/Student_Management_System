...
if (empty($un)) {
    ...
    $s = Database::select("SELECT * FROM `acedamic` WHERE `user_name`='" . $un . "'");
    $sn = $s->num_rows;
    ...
    if ($sn != 1) {
        ...
        <h4>Dear Academic Officer,</h4>
            ...
            if (!$mail->send()) {
                ...
                Database::uid("UPDATE `acedamic` SET `code`='" . $code . "' WHERE `acedamic_id`='" . $sr["acedamic_id"] . "'");//update academic
                echo "success";
            }
